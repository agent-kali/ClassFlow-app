 ...
 * teacher's whole schedule against `asOf`, never by the browsed period, and
 * kept to two lines so it reads as a banner rather than a second lesson card.
 */
export function NextLessonBanner({
  ...
  return (
    ...
    >
      ...
      <p className="teacher-next__line">
        <span className="teacher-next__time cf-mono">
          {lesson.date !== today && (
        ...
        <span className="teacher-next__class">
          {group?.program}
          <span className="cf-mono">{group?.code}</span>
        </span>
        <span className="teacher-next__where">
